  ...
  return (
    ...
    >
      <div
        ...
        onMouseEnter={() => setIsShrinkHover(true)}
        onMouseLeave={() => setIsShrinkHover(false)}
        className="bg-[#1A1A1A] hover:bg-[#252622] transition-all ease-in-out cursor-pointer w-[130px] absolute -top-5 left-1/2 transform -translate-x-1/2 h-[110px] border border-[#FAFAFA14] rounded-[35px] rotate-45"
      >
        {isShrinkHover && <Tooltip />}
        <img
          className="absolute rotate-[135deg] top-4 left-4"
          src={getImage("upArrow.svg")}
          alt=""
        />
      </div>
      <motion.div
        animate={{
          height: isShrinked ? 290 : 752,
        }}
        className="rounded-[20px] relative bg-[#191919] p-6 overflow-hidden border border-[#FAFAFA14]"
      >
        {/* grid video */}
        <video
          className="absolute h-[266px] w-full object-cover left-0 right-0 top-0"
          autoPlay
          muted
          loop
        >
        ...
        <div className="relative">
          ...
          <div className="grid justify-center space-y-2">
            ...
            <div className="flex items-center max-w-[120px] w-full mx-auto cursor-pointer hover:bg-[#FAFAFA14] transition-all rounded-[12px] p-2 ease-in-out justify-center gap-1">
              <img
                className="rounded-[6px] mr-1"
                src={getImage("grayTria.svg")}
                alt=""
              />
              <p className="text-[#808080] text-xs font-semibold">Open Tria</p>
              <img
                className="cursor-pointer"
                src={getImage("linkArrow.svg")}
                alt=""
              />
            </div>
          </div>
