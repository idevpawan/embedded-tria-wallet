...
function TokenItem(props: TTokenItem) {
  ...
  return (
    <div className="hover:bg-[#252622] transition-all ease-in-out p-2.5 rounded-[12px]">
      <div
        onClick={() =>
          props.subDetails.length && setIsDetailOpened(!isDetailOpened)
        }
        className={`flex justify-between items-center ${
          props.subDetails.length && " cursor-pointer"
        }`}
      >
            ...
            <div className="flex ml-1">
              {props.subDetails.slice(0, 3).map((item, i) => {
                return (
                  <div
                    key={i}
                    className="border bg-[#191919] -ml-1 p-1 border-[#FAFAFA14] w-6 h-6 rounded-full"
                  >
                    {i !== 2 && (
                      <img
        ...
        <div className="flex gap-3">
          <div>
            <p className="font-semibold text-right flex items-center gap-2 text-[#FAFAFA]">
      ...
      {isDetailOpened &&
        props.subDetails.map((item) => {
          return (
            <div
              key={item.id}
              className="border-t flex justify-between items-center border-[#252622] my-4"
            >
